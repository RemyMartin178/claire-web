...
/**
 * POST /api/v1/auth/mobile-auth/exchange
 * Exchange mobile session ID for custom token
 */
router.post('/mobile-auth/exchange', asyncHandler(async (req, res) => {
  const { session_id } = req.body;

  console.log('📱 [Mobile Auth] Exchange request received for session:', session_id);

  if (!session_id) {
    throw new ValidationError('Session ID is required', { field: 'session_id' });
  }

  try {
    // Read session from Firestore
    console.log('📱 [Mobile Auth] Reading session from Firestore...');
    ...
    if (!sessionDoc.exists) {
      ...
      throw new ValidationError('Session not found or expired', { code: 'SESSION_NOT_FOUND' });
    }
    ...
    if (!sessionData.uid) {
      console.log('❌ [Mobile Auth] No UID in session data');
      throw new ValidationError('Invalid session data', { code: 'INVALID_SESSION' });
    }
    
    // Check if already used
    if (sessionData.used) {
      console.log('⚠️ [Mobile Auth] Session already used');
      throw new ValidationError('Session already used', { code: 'SESSION_USED' });
    }
    ...
    throw new ValidationError('Failed to exchange session', { 
