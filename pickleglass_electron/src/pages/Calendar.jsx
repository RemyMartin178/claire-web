...
export default function Calendar({ navigate }) {
  ...
  const handleConnect = async () => {
    ...
    try { await connectCalendar(user.uid) } catch (e) { console.error(e) }
  ...
  const groupedEvents = useMemo(() => {
    ...
    return Array.from(groups.values())
      .map(g => ({ ...g, events: g.events.sort((a,b) => (getEventStartDate(a)?.getTime()||0) - (getEventStartDate(b)?.getTime()||0)) }))
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-3xl mx-auto px-6 py-14">
        ...
        {!isConfigured ? (
          <div className="mt-2">
            <p className="text-sm text-gray-500 mb-5">Connectez votre compte Google pour synchroniser vos réunions.</p>
            <button
                        ...
                        return (
                          ...
                          >
                            <div className="flex-1 min-w-0 pr-4">
                              <span className="text-[15px] font-medium text-black block truncate">{getEventTitle(event)}</span>
                            </div>
                            <div className="flex items-center gap-3 shrink-0">
                              <span className="text-sm font-mono text-neutral-500 bg-neutral-100 px-2 py-0.5 rounded-md hidden sm:block">{duration}</span>
