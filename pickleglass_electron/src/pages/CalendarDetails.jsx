...
function formatAttendeeLabel(label) {
  ...
  const name = local.split(/[._-]/).filter(Boolean)
    .map(p => p.charAt(0).toUpperCase() + p.slice(1).replace(/\d+$/, '')).join(' ').trim() || label
...
export default function CalendarDetails({ navigate, event }) {
  ...
  if (!event) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6">
        <p className="text-[15px] text-gray-400 mb-4">Réunion introuvable.</p>
        <button onClick={() => navigate('calendar')} className="text-sm font-medium text-blue-500 hover:underline">
  ...
  const handlePrepareEmail = async () => {
    setIsEmailing(true)
    try {
      const subject = encodeURIComponent(`Suivi de réunion : ${title}`)
      const body = encodeURIComponent(`Bonjour,\n\nSuite à notre réunion "${title}", voici un suivi...\n\nCordialement`)
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <span className="flex items-center gap-1.5 text-[13px] font-medium text-[#1d1d1f] bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5">
            <CalendarIcon />{formatEventDateLabel(event)}
          </span>
          <span className="flex items-center gap-1.5 text-[13px] font-medium text-[#1d1d1f] bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5">
            <ClockIcon />{formatEventRangeLabel(event)} ({formatDurationLabel(event)})
          </span>
          {joinLink && (
        ...
        {activeTab === 'summary' ? (
          <div className="space-y-8">
            {/* Description */}
            <div>
              <h2 className="text-[17px] font-semibold text-[#1d1d1f] mb-3">Résumé</h2>
              {event.description ? (
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-gray-100 p-5 bg-white space-y-3">
              <div className="flex items-center gap-3 text-[14px] text-[#4b5563]">
                <CalendarIcon /><span>{formatEventDateLabel(event)}</span>
              </div>
              <div className="flex items-center gap-3 text-[14px] text-[#4b5563]">
                <ClockIcon /><span>{formatEventRangeLabel(event)} ({formatDurationLabel(event)})</span>
              </div>
              {organizer && (
                <div className="flex items-center gap-3 text-[14px] text-[#4b5563]">
                  <UsersIcon /><span>Organisateur : {organizer}</span>
                </div>
              )}
              {event.location && (
                <div className="flex items-center gap-3 text-[14px] text-[#4b5563]">
            ...
            {(joinLink || event.htmlLink) && (
              <div className="rounded-2xl border border-gray-100 p-5 bg-white space-y-3">
                ...
                {joinLink && (
                  <a href={joinLink} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 text-[14px] text-[#1d1d1f] hover:text-[#007aff] transition-colors"
                  >
                    <VideoIcon />Rejoindre la réunion<ExternalLinkIcon />
                  </a>
                )}
                {event.htmlLink && (
                  <a href={event.htmlLink} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 text-[14px] text-[#1d1d1f] hover:text-[#007aff] transition-colors"
                  >
                    <CalendarIcon />Ouvrir dans Google Calendar<ExternalLinkIcon />
                  </a>
                )}
              </div>
            )}
