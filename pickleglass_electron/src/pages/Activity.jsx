  ...
  return (
    <div className="h-full bg-white">
      <div className="max-w-[760px] mx-auto px-5 py-8">
        <h1 className="text-[28px] font-semibold text-black mb-2 leading-tight">
          {greeting()}, {user?.displayName?.split(' ')[0] || 'toi'}
        </h1>
        <p className="text-[13px] text-gray-500 mb-5">Historique compact de tes sessions Claire dans la version Electron.</p>

        <button
          onClick={handleStartClaire}
          disabled={launching}
          className="mb-7 flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-60"
        ...
        >
          ...
          {launching ? 'Lancement...' : 'Demarrer Claire'}
        </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-14 border border-dashed border-neutral-200 rounded-2xl bg-neutral-50/50">
          ...
          <div className="space-y-8">
            {sortedDates.map(dateStr => (
              <div key={dateStr}>
                <h3 className="text-[12px] font-semibold text-gray-500 mb-3 tracking-wide">{dateStr}</h3>
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 min-w-0 pr-4">
                          <span className="text-[14px] font-medium text-black block truncate">{title}</span>
                        </div>

                        <div className="flex items-center gap-4 shrink-0">
                          <span className="text-[12px] font-mono text-neutral-500 bg-neutral-100 px-2 py-0.5 rounded-md">{duration}</span>
