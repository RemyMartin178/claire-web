    ...
    setupServiceCallbacks() {
        ...
        this.sttService.setCallbacks({
            ...
            onSilenceTimeout: async () => {
                logger.info('[ListenService] Silence timeout — auto-ending session');
                if (notificationManager) {
                    notificationManager.showNotification(
                        'Aucune transcription détectée',
                        'Claire a mis fin à votre session automatiquement',
                        { tag: 'session-auto-end', icon: 'mic' }
                    );
                }
                await this.handleListenRequest('Stop').catch(() => {});
            },
        });
    ...
    setupIpcHandlers() {
        ...
        ipcMain.handle('listen:pause-microphone', async (event) => {
            try {
                ...
                this.sendToRenderer('listen-pause-state-changed', { isPaused: true });
        ...
        ipcMain.handle('listen:resume-microphone', async (event) => {
            try {
                ...
                this.sendToRenderer('listen-pause-state-changed', { isPaused: false });
    ...
    async initializeSession(language = 'auto') {
        ...
        try {
            ...
            try {
                ...
                logger.info('[OK] System audio capture started');
            } catch (error) {
                logger.warn('[ERROR] System audio initialization error (continuing anyway):', error.message);
            }
