...
const { resourcePoolManager } = require('../../services/resource-pool-manager.js');
...
class OpenAIProvider {
    static async validateApiKey(key) {
        ...
        try {
            // Use ResourcePoolManager to prevent EPIPE errors during API key validation
            logger.debug('[OpenAI] Making API key validation request via ResourcePoolManager');
            const response = await resourcePoolManager.queuedFetch('https://api.openai.com/v1/models', {
...
async function createSTT({ apiKey, language = null, callbacks = {}, usePortkey = false, portkeyVirtualKey, ...config }) {
  ...
  if (!key || typeof key !== 'string') {
    const errorMsg = `OpenAI STT: No ${keyType} provided. Required for STT sessions.`;
    logger.error(errorMsg, { keyType, hasKey: !!key });
    throw new Error(errorMsg);
  }

  logger.info('Creating OpenAI STT session:', { 
    keyType, 
    language, 
    hasKey: !!key,
  ...
  const headers = keyType === 'apiKey'
    ...
    : {
        'x-portkey-api-key': process.env.PORTKEY_API_KEY || 'gRv2UGRMq6GGLJ8aVEB4e7adIewu',
  ...
  return new Promise((resolve, reject) => {
    ws.onopen = () => {
      ...
      const sessionConfig = {
        ...
        session: {
          ...
          turn_detection: {
            type: 'server_vad',
            threshold: 0.5, // Original setting for faster responsiveness
            prefix_padding_ms: 200, // Original setting for faster processing
            silence_duration_ms: 100, // Original setting for real-time like glass_old
          },
      ...
      const keepAlive = () => {
        try {
          ...
          logger.error('[OpenAI STT] keepAlive error:', err.message);
        }
      };
      ...
      resolve({
        sendRealtimeInput: (audioData) => {
          if (ws.readyState === WebSocket.OPEN) {
            ...
            if (Math.random() < 0.1) { // Log only ~10% of audio chunks
              logger.info(`Sent audio chunk: ${audioData.length} bytes`);
            }
          }
        },
        ...
        close: () => {
          if (ws.readyState === WebSocket.OPEN) {
            ...
            ws.onmessage = ws.onerror = () => {};  // [Korean comment translated] [Korean comment translated]
    ...
    ws.onmessage = (event) => {
      // ── Shutdown·[Korean comment translated] [Korean comment translated] [Korean comment translated] ──────────────────────────────
      if (!event.data || event.data === 'null' || event.data === '[DONE]') return;

      let msg;
      try { msg = JSON.parse(event.data); }
      catch { return; }                       // JSON [Korean comment translated] Failed [Korean comment translated]

      if (!msg || typeof msg !== 'object') return;

      msg.provider = 'openai';                // ← [Korean comment translated] [Korean comment translated]
      callbacks.onmessage?.(msg);
    };

    ws.onerror = (error) => {
      logger.error('Error occurred', { message: 'WebSocket error', error: error.message });
 ...
 * @param {object} opts - Configuration options
 * @param {string} opts.apiKey - OpenAI API key
 * @param {string} [opts.model='gpt-4'] - Model name
 ...
 */
function createLLM({ apiKey, model = 'gpt-4', temperature = 0.7, maxTokens = 2048, usePortkey = false, portkeyVirtualKey, ...config }) {
 ...
 */
function createStreamingLLM({ apiKey, model = 'gpt-4', temperature = 0.7, maxTokens = 2048, usePortkey = false, portkeyVirtualKey, ...config }) {
  return {
