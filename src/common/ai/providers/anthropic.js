...
async function createSTT({ apiKey, language = "en", callbacks = {}, ...config }) {
  logger.warn('STT not natively supported. Consider using OpenAI or Gemini for STT.')
  ...
  return {
    sendRealtimeInput: async (audioData) => {
      logger.warn('STT sendRealtimeInput called but not implemented')
    },
    close: async () => {
      logger.info("[Anthropic] STT session closed")
    },
  }
}
...
function createStreamingLLM({
  ...
  return {
    streamChat: async (messages) => {
      logger.info("[Anthropic Provider] Starting streaming request")
          ...
          if (Array.isArray(msg.content)) {
            ...
            for (const part of msg.content) {
              if (typeof part === "string") {
                ...
                logger.info('Processing image with MIME type:')
      ...
      const stream = new ReadableStream({
        async start(controller) {
          try {
            logger.info("[Anthropic Provider] Processing messages:", anthropicMessages.length, "messages")
            ...
            controller.close()
            logger.info("[Anthropic Provider] Streaming completed successfully")
          } catch (error) {
