...
class ModelStateService extends EventEmitter {
    ...
    async initialize() {
        ...
        // Reload API keys whenever the user logs in or out
        this.authService.onUserStateChanged(async (userState) => {
            logger.info('[ModelStateService] Auth state changed, reloading state for user:', userState.uid);
            await this._loadStateForCurrentUser();
        ...
        try {
            ...
            for (const [provider, envKey] of Object.entries(envMapping)) {
                ...
                if (envKey && envKey.trim() !== '') {
                    ...
                    apiKeys[provider] = null;
                    logger.debug(`[ModelStateService] No ${provider} API key in environment or database`);
                }
            }
