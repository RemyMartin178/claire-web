...
class WindowLayoutManager {
    constructor(windowPool, movementManager) {
    ...
    positionWindows() {
        ...
        if (settings && !settings.isDestroyed() && settings.isVisible()) {
            ...
            if (settingPos) {
                if (this.movementManager && this.movementManager.animateWindow) {
                    this.movementManager.animateWindow(settings, settingPos.x, settingPos.y, { duration: 300 });
                } else {
        ...
        if (agentSelector && !agentSelector.isDestroyed() && agentSelector.isVisible()) {
            ...
            if (agentSelectorPos) {
                if (this.movementManager && this.movementManager.animateWindow) {
                    this.movementManager.animateWindow(agentSelector, agentSelectorPos.x, agentSelectorPos.y, { duration: 300 });
                } else {
    ...
    calculateSettingsWindowPosition() {
        ...
        const isAskVisible = ask && !ask.isDestroyed() && ask.isVisible();
        if (isAskVisible) {
            const askBounds = ask.getBounds();
            x = askBounds.x + askBounds.width + PAD;
            y = askBounds.y; // Align vertically with Ask
            
            // If exceeding the right boundary, push to the left side of Ask
            if (x + settingsBounds.width + 10 > workAreaX + screenWidth) {
                x = askBounds.x - settingsBounds.width - PAD;
            }
        }
    ...
    calculateAgentSelectorWindowPosition() {
        ...
        let x = headerBounds.x + headerBounds.width - agentSelectorBounds.width - agentButtonPadding;
        let y = headerBounds.y + headerBounds.height + PAD;
        ...
        if (isAskVisible) {
            ...
            if (x + agentSelectorBounds.width + 10 > workAreaX + screenWidth) {
                x = askBounds.x - agentSelectorBounds.width - PAD;
            }
        }
