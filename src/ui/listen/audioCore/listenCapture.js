...
let AUDIO_CHUNK_DURATION = 0.025; // Default: 25ms (Deepgram). AssemblyAI needs min 50ms (set at startCapture)
let _lastKnownProvider = null; // Persists across startCapture calls so no-arg callers don't reset chunk size
...
async function startCapture(screenshotIntervalSeconds = 5, imageQuality = 'medium', provider = null) {
    ...
    try {
        if (isMacOS()) {
            ...
            try {
                // Close any existing system audio context to prevent ghost 25ms processors
                if (systemAudioContext) {
                    try { systemAudioContext.close(); } catch (_) {}
...
if (typeof window !== 'undefined') {
    ...
    if (window.api && window.api.on) {
        ...
        window.api.on('start-system-audio-capture', async (_event, { provider } = {}) => {
