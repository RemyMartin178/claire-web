...
const inferPlanFromSubscription = (sub: Stripe.Subscription): 'free' | 'plus' | 'max' | 'enterprise' => {
  ...
  const maxMonthlyId = process.env.STRIPE_MAX_MONTHLY_PRICE_ID
  const maxAnnualId = process.env.STRIPE_MAX_ANNUAL_PRICE_ID
  const enterpriseId = process.env.STRIPE_ENTERPRISE_PRICE_ID

  if (priceId && enterpriseId && priceId === enterpriseId) return 'enterprise'
  if (priceId && (priceId === maxMonthlyId || priceId === maxAnnualId)) return 'max'
    ...
    switch (event.type) {
        ...
        try {
          ...
          if (userData?.email) {
            ...
            const endDate = currentPeriodEnd
              ...
              : ''
            await sendEmail(userData.email, 'subscriptionConfirmed', userData.displayName || '', plan, endDate)
          }
        } catch {}
        break
      }
      ...
      case 'customer.subscription.updated': {
        ...
        try {
          ...
          if (userDoc) {
            ...
            // Mettre à jour avec les vraies dates Stripe — toujours écraser pour éviter les dates périmées
            await userDoc.ref.update({
              'subscription.currentPeriodStart': currentPeriodStart ?? null,
        ...
        try {
          ...
          if (userDoc) {
            ...
            try {
              const userData = userDoc.data()
              if (userData?.email) {
                const endDate = safeDateFromStripeSeconds((subscription as any).current_period_end)
                const endDateStr = endDate
                  ? endDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })
                  : ''
                await sendEmail(userData.email, 'subscriptionCancelled', userData.displayName || '', endDateStr)
              }
            } catch {}
          } else {
      ...
      case 'invoice.payment_succeeded': {
        ...
        try {
          ...
          let plan: 'free' | 'plus' | 'max' | 'enterprise' | undefined
          ...
          try {
            ...
            if (userData?.email) {
              const nextRetry = (invoice as any).next_payment_attempt
                ? new Date((invoice as any).next_payment_attempt * 1000).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })
                : undefined
              await sendEmail(userData.email, 'paymentFailed', userData.displayName || '', nextRetry)
            }
          } catch {}
        } catch (e) {
      ...
      case 'charge.refunded': {
        ...
        try {
          // Cancel the active subscription on Stripe
            ...
            try {
              ...
              if (userData?.email) {
                const amount = charge.amount_refunded
                  ? `${(charge.amount_refunded / 100).toFixed(2)} ${(charge.currency || 'eur').toUpperCase()}`
                  : ''
                await sendEmail(userData.email, 'refundConfirmed', userData.displayName || '', amount)
              }
            } catch {}
          }
        } catch (error) {
          console.error('❌ charge.refunded handling failed:', error)
        }
        break
      }
