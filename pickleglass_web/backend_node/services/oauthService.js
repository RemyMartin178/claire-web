...
class OAuthService {
   ...
   */
  generateAuthUrl(toolName, providerName = null, redirectUri = null, userId = null, platform = null) {
    try {
      ...
      // Create state with user ID and optional platform for proper session tracking
      let state = `${toolName}:${Date.now()}`;
      if (userId) {
        state = `${toolName}:${userId}:${Date.now()}${platform ? `:${platform}` : ''}`;
      }
  ...
  async refreshAccessToken(toolName, refreshToken, providerName = null) {
    try {
      ...
      // Create a token object from the refresh token
      ...
      return {
        access_token: result.token.access_token,
        refresh_token: result.token.refresh_token || refreshToken, // Some providers don't return a new refresh token
        expires_at: result.token.expires_at || null,
        scope: result.token.scope || null,
        token_type: result.token.token_type || 'Bearer'
      };
    } catch (error) {
      throw new Error(`Failed to refresh token for ${toolName}: ${error.message}`);
    }
  }
}
