 ...
 * Simple, direct Google Calendar API integration for Claire
 */
...
class GoogleCalendarTool {
  ...
  async listEvents(parameters = {}) {
    try {
      ...
      console.log(`[GoogleCalendarTool] Listing events for calendar: ${calendarId}, timeMin: ${timeMin}`);
      ...
      console.log(`[GoogleCalendarTool] Found ${events.length} events from Google`);
  ...
  /**
   * Get user profile information (email, name, etc.)
   */
  async getUserProfile() {
    try {
      ...
      const oauth2Client = this.calendar._options.auth;
  ...
  async testConnection() {
    try {
      ...
      // Also try to get user profile if scopes permit
      let userEmail = null;
      try {
        const profile = await this.getUserProfile();
        if (profile.success) {
          userEmail = profile.profile.email;
          console.log(`[GoogleCalendarTool] Fetched email from profile: ${userEmail}`);
        }
      } catch (e) {
        console.log('[GoogleCalendarTool] Could not fetch profile, falling back to calendar ID');
      }

      // If profile failed, use the calendar ID if it looks like an email
      if (!userEmail && response.data.id && response.data.id.includes('@')) {
        userEmail = response.data.id;
        console.log(`[GoogleCalendarTool] Falling back to calendar ID as email: ${userEmail}`);
      }
  ...
  getAvailableOperations() {
    return [
      ...
      {
        name: 'getUserProfile',
        description: 'Get connected user profile (email, name)',
        parameters: {}
      },
      {
        name: 'createEvent',
