...
function LoginContent() {
  ...
  const associateAfterLogin = async (nextSessionId: string) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'pending_sessions', nextSessionId), {
        uid: user.uid,
        email: user.email,
        created_at: new Date(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const signedInUser = await signInWithEmail(formData.email, formData.password, formData.rememberMe)

      if (isMobileFlow && sessionId && signedInUser) {
        await associateAfterLogin(sessionId)
      }
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (isMobileFlow && sessionId && result) {
        await associateAfterLogin(sessionId)
      }
      ...
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="animate-scale-in w-full max-w-lg">
          <div className="text-center mb-6 animate-fade-in">
            <h2 className="text-2xl font-heading font-semibold text-[#282828] mb-2">Connexion Claire</h2>
          </div>
