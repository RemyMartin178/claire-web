...
import { getAuthType, getApiHeaders } from '@/utils/api'
...
export default function SecurityPage() {
  ...
  // Récupérer les appareils connectés depuis le backend
  useEffect(() => {
    if (userInfo) {
      detectAuthType();
      ...
      const fetchSessions = async () => {
        try {
          ...
          // Récupérer l'URL du backend depuis le runtime config ou fallback
          let baseUrl = 'http://localhost:3001';
          try {
            const configRes = await fetch('/runtime-config.json');
            if (configRes.ok) {
          ...
          const response = await fetch(`${baseUrl}/api/v1/sessions/user/${userInfo.uid}`, { headers });
          if (response.ok) {
            const sessionsData = await response.json();

            // Map the backend data to the local Device interface
            const formattedDevices: Device[] = sessionsData.map((session: any) => ({
              id: session.id,
              name: session.os_info?.includes('Windows') ? 'PC Windows' : session.os_info?.includes('Mac') ? 'Mac' : 'Appareil Inconnu',
              os: session.os_info || 'Unknown',
              browser: session.browser_info || 'Unknown',
              location: 'Localisation masquée',
              ip: session.ip_address || 'Non spécifiée',
              lastSeen: new Date(session.last_seen_at).toLocaleString('fr-FR'),
              isCurrent: false // Backend needs to tell us or we check current session ID
            }));

            // Pour l'instant on garde le mock du current device + les sessions du backend
      ...
      const initFallbackDevice = async () => {
        try {
          ...
          setDevices([{
            id: 'current',
            name: 'Cet appareil',
            os: navigator.platform.includes('Win') ? 'Windows' : navigator.platform.includes('Mac') ? 'macOS' : 'Unknown',
  ...
  const handleDisconnectDevice = async (deviceId: string) => {
    // Without a server-side session table, revoke all tokens then re-authenticate current device
    await handleDisconnectAllDevices(deviceId)
  };

  const handleDisconnectAllDevices = async (exceptDeviceId?: string) => {
    try {
      ...
      // Force refresh token for current device so it stays logged in
      await currentUser.getIdToken(true)

      setDevices(prev => prev.filter(d => d.isCurrent || d.id === exceptDeviceId))
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-transparent shadow-none border-neutral-200 dark:border-neutral-800 transition-colors">
          <CardContent className="p-6">
            <h3 className="text-lg font-heading font-semibold text-[#282828] mb-1">Adresse Email</h3>
            <p className="text-sm text-gray-600 mb-4">L'adresse email associée à votre compte utilisé pour la connexion et les notifications.</p>

            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="font-medium text-gray-900">{userInfo?.email || 'Chargement...'}</p>
              <p className="text-xs text-gray-500 mt-1">Pour modifier votre adresse email, veuillez contacter le support.</p>
            </div>
          </CardContent>
        </Card>
