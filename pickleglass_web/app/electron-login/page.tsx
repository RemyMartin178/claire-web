...
type AuthState = 'idle' | 'loading' | 'waitingDeepLink' | 'success'
...
export default function ElectronLoginPage() {
  ...
  useEffect(() => {
    ...
    const syncCurrentUser = async () => {
      try {
        ...
        if (!cancelled && (result as { user?: unknown } | null)?.user) {
          ...
          window.setTimeout(() => {
            if (!cancelled) router.replace('/activity')
          }, 600)
        }
      } catch {
        // noop
    ...
    const handleUserChanged = (state: { isLoggedIn?: boolean; user?: unknown; uid?: string; email?: string; displayName?: string | null }) => {
  ...
  }, [authState, isElectronRuntime, router])
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!window.api?.common?.startFirebaseAuth) {
  ...
  const isWaiting = authState === 'waitingDeepLink'
  ...
  return (
    <div
      style={{
        ...
        overflow: 'hidden',
        fontFamily:
          '"Google Sans", "Plus Jakarta Sans", -apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Helvetica Neue", sans-serif',
        background: '#ffffff',
      }}
    >
      <div
        data-electron-right-panel="true"
        style={{
          flex: '1 1 50%',
          display: 'flex',
      ...
      >
        ...
        >
          ...
          >
            Bienvenue sur <span style={{ color: '#1d1d1f' }}>Claire</span>
          </h1>
          ...
          >
            {isWaiting
              ? 'Connexion en cours dans votre navigateur. Revenez ici apres validation.'
              : isSuccess
                ? 'Connexion validee. Ouverture du dashboard...'
                : 'Le flux de connexion ouvre votre navigateur puis revient ici via deeplink.'}
          </p>
          ...
          >
            <button
              ...
              disabled={isBusy || isElectronRuntime !== true}
              style={{
                width: '100%',
                maxWidth: 320,
                height: 58,
                borderRadius: 999,
                border: '1px solid #0c44a1',
                ...
                boxShadow: '0 0 0 0.678px #0c44a1, inset 0 -1.355px #022c70, inset 0 0.678px #81b6ff',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: 10,
          ...
          >
            <div
              style={{
                ...
                pointerEvents: 'none',
                background:
                  'linear-gradient(90deg, transparent, rgba(255,255,255,0.18) 30%, rgba(255,255,255,0.22) 50%, rgba(255,255,255,0.18) 70%, transparent)',
              }}
            />
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 3, height: 14, marginLeft: 6, marginRight: 4 }}>
              {[0.35, 1, 0.6, 0.85, 0.4].map((scale, index) => (
                <div
                  key={index}
                  style={{
                    width: 2.5,
            ...
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" style={{ opacity: 0.75 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '20px 16px 8px' }}>
              <div
                style={{
                  background: 'radial-gradient(179.05% 132.83% at 46.18% -23.44%, #1562df 0, #0c26a8 100%)',
                  color: '#CBE3FF',
                  boxShadow: '0 0 0 .678px #0c44a1, inset 0 -1.355px #022c70, inset 0 .678px #81b6ff',
              ...
              >
                Claire peut-elle resumer ma reunion en direct ?
              </div>
            </div>
            ...
            <div style={{ padding: '8px 16px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              <p style={{ fontSize: 13.5, lineHeight: 1.65, color: 'rgba(255,255,255,0.78)', margin: '0 0 10px' }}>
                Oui, Claire ecoute votre reunion en temps reel et genere un{' '}
                <strong style={{ color: 'rgba(255,255,255,0.92)', fontWeight: 600 }}>resume live</strong>{' '}
                avec les decisions, actions a suivre et points cles.
              </p>
              <p style={{ fontSize: 13.5, lineHeight: 1.65, color: 'rgba(255,255,255,0.78)', margin: 0 }}>
                Vous pouvez aussi poser des questions sur ce qui vient d&apos;etre dit, demander une{' '}
            ...
            >
              ...
              >
                Posez une question sur votre ecran ou la conversation...
              </span>
              <div
                style={{
                  display: 'flex',
              ...
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                  <path d="M9.663 17h4.673M12 3v1m6.364 1.636-.707.707M21 12h-1M4 12H3m1.636-6.364.707.707M6.343 17.657l-.707.707" />
              ...
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#CBE3FF" strokeWidth="2">
      ...
      <style>{`
        ...
        @media (max-width: 1100px) {
          div[data-electron-right-panel="true"] {
            display: none;
          }
        }
      `}</style>
